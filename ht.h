...
public:
    ...
    HASH_INDEX_T next() 
    {
        // Store constant values to avoid multiple lookups
        const HASH_INDEX_T maxProbes = this->m_;
        const HASH_INDEX_T npos = this->npos;

        // Check if the maximum number of probes is reached
        if (this->numProbes_ == maxProbes) {
            return npos;
        }

        // Calculate the next probe location using double hashing
        HASH_INDEX_T location = (this->start_ + this->numProbes_ * dhstep_) % maxProbes;
...
{
    double adj_index = static_cast<double>(vector_size) + counter;
    auto flag = internalFind(p.first);
    bool resize_needed = (loading_factor <= adj_index / table_.size());

    if (resize_needed) {
        resize();
    }

    // Update table size after potential resize
    size_t tsize = table_.size();
    auto probe_index = this->probe(p.first);

    if (tsize <= probe_index) {
